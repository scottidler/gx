...
//! [`crate::schema`]. Every function here is synchronous and blocking (it calls
//! gx cores that shell out to git/gh and build rayon pools); the async tool
//! wrappers in [`crate::server`] run each under `tokio::task::spawn_blocking` so
...
use gx::confirm::Confirmation;
use gx::create::manifest::{FileAction, ProposalManifest, ProposalOutcome};
use gx::git::RemoteStatus;
use gx::state::StateManager;
use gx::undo::core::UndoPlanSet;
use log::debug;
use std::env;
use std::path::Path;
...
fn jobs(config: &Config) -> usize {
    gx::utils::get_jobs_from_config(config)
        .or_else(gx::utils::get_nproc)
        .unwrap_or(4)
}

/// Effective repo-discovery max depth (config, else 2 as `gx status` uses).
fn max_depth(config: &Config) -> usize {
    gx::utils::get_max_depth_from_config(config).unwrap_or(2)
}

/// Discover + filter the fleet under the server's CWD by the given patterns.
fn discover(config: &Config, patterns: &[String]) -> Result<Vec<gx::repo::Repo>> {
    let start_dir = env::current_dir()?;
    let repos = gx::repo::discover_repos(&start_dir, max_depth(config), &config.ignore_patterns())?;
    Ok(gx::repo::filter_repos(repos, patterns))
}
...
pub fn status(
    ...
    Ok(repos
        .iter()
        .map(|repo| {
            let rs = gx::git::get_repo_status_with_options(repo, false, no_remote);
            RepoStatusSummary {
...
pub fn change_get(change_id: &str, slug: Option<&str>) -> Result<ChangeDetail> {
    ...
    let dir = gx::create::manifest::proposal_dir(change_id)?;
    let proposal = if dir.join("manifest.json").exists() {
        let manifest = gx::create::manifest::load_manifest(&dir)?;
...
fn proposal_detail(dir: &Path, manifest: &ProposalManifest, only: Option<&str>) -> ProposalDetail {
    let repos = manifest
        ...
        .map(|rp| {
            let patch_path = gx::create::manifest::patch_path(dir, &rp.slug);
...
pub fn doctor() -> Result<gx::doctor::DoctorReport> {
    debug!("logic::doctor");
    gx::doctor::collect_report()
}
...
pub fn create_propose(config: &Config, prompt: &str, patterns: &[String]) -> Result<ProposeOut> {
    ...
    let change_id = gx::create::generate_change_id();
    let summary = gx::create::core::propose::execute_propose(
        &repos,
        &change_id,
        prompt,
        config,
...
pub fn undo_plan_token(plan: &UndoPlanSet) -> String {
    ...
    let hex = gx::hash::sha256_hex(canon.as_bytes());
    hex.chars()
        .take(gx::create::manifest::TOKEN_HEX_LEN)
        .collect()
}

pub fn undo_plan(config: &Config, change_id: &str) -> Result<UndoPlanOut> {
    debug!("logic::undo_plan: change_id={change_id}");
    let Some(plan_set) = gx::undo::core::plan_undo(change_id, None, config)? else {
...
pub fn undo_execute(config: &Config, change_id: &str, token: &str) -> Result<UndoExecuteOut> {
    ...
    let outcomes = gx::undo::core::execute_undo(
        &plan_set,
        change_id,
        config,
        jobs(config),
