...
impl SshUrlBuilder {
    ...
    pub fn build_ssh_url(repo_slug: &str) -> Result<String> {
        ...
        if parts.len() != 2 {
            return Err(eyre::eyre!("Invalid repository slug format. Expected 'org/repo', got '{}'", repo_slug));
        }
        ...
        Ok(format!("git@github.com:{}.git", repo_slug))
    }

    /// Validate SSH URL format
    pub fn validate_ssh_url(url: &str) -> Result<()> {
        if !url.starts_with("git@github.com:") {
            return Err(eyre::eyre!("Invalid SSH URL format. Expected to start with 'git@github.com:', got '{}'", url));
        }

        if !url.ends_with(".git") {
            return Err(eyre::eyre!("Invalid SSH URL format. Expected to end with '.git', got '{}'", url));
        }
...
impl SshCommandDetector {
    ...
    pub fn test_github_ssh_connection() -> Result<String> {
        ...
        if stderr.contains("Permission denied") {
            return Err(eyre::eyre!("SSH authentication failed: Permission denied. Check your SSH keys."));
        }
        ...
        if stderr.contains("Connection refused") || stderr.contains("Network is unreachable") {
            return Err(eyre::eyre!("SSH network error: Cannot connect to GitHub. Check your network connection."));
        }

        // Generic failure
        Err(eyre::eyre!("SSH connection test failed. stderr: {}, stdout: {}", stderr.trim(), stdout.trim()))
    }
}
...
mod tests {
    ...
    fn test_build_ssh_url_invalid_format() {
        ...
        assert!(result.unwrap_err().to_string().contains("Invalid repository slug format"));
    }
    ...
    fn test_build_ssh_url_empty_parts() {
        ...
        assert!(result.is_err());
        assert!(result.unwrap_err().to_string().contains("Repository slug parts cannot be empty"));
    ...
    fn test_validate_ssh_url_invalid_prefix() {
        ...
        assert!(result.is_err());
        assert!(result.unwrap_err().to_string().contains("Expected to start with 'git@github.com:'"));
    }
