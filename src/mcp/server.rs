...
impl ServerHandler for GxMcpServer {
    fn get_info(&self) -> ServerInfo {
        ...
        // Without an explicit server_info, rmcp reports the server name as "rmcp"
        // (ServerInfo::new defaults to Implementation::from_build_env(), which
        // bakes rmcp's own crate name). `gx mcp status` warns on that mismatch,
        // so name it "gx" here. Version rides gx's CARGO_PKG_VERSION (the same
        // value mcp_io!() captures for registration/bundle).
        let mut info = ServerInfo::new(capabilities)
            .with_server_info(Implementation::new("gx", env!("CARGO_PKG_VERSION")));
