...
pub fn create_pr(repo_slug: &str, branch_name: &str, commit_message: &str) -> Result<()> {
    debug!("Creating PR for repo: {}, branch: {}", repo_slug, branch_name);
...
pub fn list_prs_by_change_id(org: &str, change_id_pattern: &str) -> Result<Vec<PrInfo>> {
    debug!("Listing PRs for org: {}, change ID pattern: {}", org, change_id_pattern);

    let output = Command::new("gh")
        .args([
            "pr",
            "list",
            "--search", &format!("org:{} head:{}", org, change_id_pattern),
    ...
    let json_output = String::from_utf8(output.stdout)
        .context("Invalid UTF-8 in gh pr list output")?;
...
pub fn approve_and_merge_pr(repo_slug: &str, pr_number: u64, admin_override: bool) -> Result<()> {
    debug!("Approving and merging PR #{} in {}", pr_number, repo_slug);
    ...
    if merge_output.status.success() {
        info!("Successfully merged PR #{} in {}", pr_number, repo_slug);
        Ok(())
    } else {
...
pub fn close_pr(repo_slug: &str, pr_number: u64) -> Result<()> {
    debug!("Closing PR #{} in {}", pr_number, repo_slug);
    ...
    if output.status.success() {
        info!("Successfully closed PR #{} in {}", pr_number, repo_slug);
        Ok(())
    } else {
        let error = String::from_utf8_lossy(&output.stderr);
...
pub fn delete_remote_branch(repo_slug: &str, branch_name: &str) -> Result<()> {
    debug!("Deleting remote branch '{}' in {}", branch_name, repo_slug);

    let output = Command::new("gh")
        .args([
            "api",
            &format!("repos/{}/git/refs/heads/{}", repo_slug, branch_name),
            "--method", "DELETE",
        ])
        .output()
    ...
    if output.status.success() {
        info!("Successfully deleted remote branch '{}' in {}", branch_name, repo_slug);
        Ok(())
    } else {
        let error = String::from_utf8_lossy(&output.stderr);
        Err(eyre::eyre!("Failed to delete remote branch '{}': {}", branch_name, error))
    }
}
...
pub fn get_pr_diff(repo_slug: &str, pr_number: u64) -> Result<String> {
    debug!("Getting diff for PR #{} in {}", pr_number, repo_slug);
    ...
    if output.status.success() {
        let diff = String::from_utf8(output.stdout)
            .context("Invalid UTF-8 in pr diff output")?;
        Ok(diff)
    } else {
        let error = String::from_utf8_lossy(&output.stderr);
...
pub fn list_branches_with_prefix(repo_slug: &str, prefix: &str) -> Result<Vec<String>> {
    debug!("Listing branches with prefix '{}' in {}", prefix, repo_slug);

    let output = Command::new("gh")
        .args([
            "api",
            &format!("repos/{}/branches", repo_slug),
            "--jq", &format!(".[] | select(.name | startswith(\"{}\")) | .name", prefix),
        ])
        .output()
